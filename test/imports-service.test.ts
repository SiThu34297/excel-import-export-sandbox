...
test('commit requires a matching validation token and revalidates before upserting', async () => {
  const file = await readFile(join(process.cwd(), 'examples/sample.xlsx'));
  ...
  const mixedPreview = await service.validateUpload(
    await readFile(join(process.cwd(), 'examples/upsert-demo.xlsx')),
    'alice',
  );
  ...
  await assert.rejects(
    service.commit(Buffer.from('changed'), result.validationToken, 'alice'),
  );
  assert.equal(
  ...
  assert.ok(
    statements.some((sql) => sql.includes('ON CONFLICT (item_code) DO UPDATE')),
  );
